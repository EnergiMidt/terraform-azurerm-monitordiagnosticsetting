...
| <a name="input_diagnostic_enabled_log_categories"></a> [diagnostic\_enabled\_log\_categories](#input\_diagnostic\_enabled\_log\_categories) | (Required) The list of Diagnostic Log Categories.<br>  The Log Categories available vary depending on the Resource being used.<br>  You may wish to use the `azurerm_monitor_diagnostic_categories` Data Source or list of service specific schemas to identify which categories are available for a given Resource. | `list(string)` | n/a | yes |
